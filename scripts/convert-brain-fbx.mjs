#!/usr/bin/env node
/**
 * Convert brain.fbx + locomotion animation FBX files into a single GLB with
 * all animations embedded and the embedded texture preserved.
 *
 * Reads:
 *   assets/source/brain.fbx  — base mesh + skeleton + embedded texture
 *   assets/source/<anim>.fbx — animation-only FBX files (explicit allowlist)
 *
 * Writes:
 *   assets/source/custom/brain.glb  (picked up by optimize-custom-glbs.mjs)
 */
...
const BASE_FBX = path.join(SOURCE_DIR, 'brain.fbx');
const OUTPUT = path.resolve('assets/source/custom/brain.glb');

// Explicit allowlist of FBX files to pull animations from. Mapped to the
// runtime clip name used by BrainAnimationManager.
const ANIM_FILES = [
  ['idle.fbx', 'idle'],
  ['jump.fbx', 'jump'],
  ['walking.fbx', 'walk'],
...
async function main() {
  if (!fs.existsSync(BASE_FBX)) {
    console.log(`No base mesh at ${BASE_FBX}, skipping brain conversion.`);
    return;
  }
  ...
  if (textureBuffer) {
    ...
    const texture = doc.createTexture('brain-diffuse')
