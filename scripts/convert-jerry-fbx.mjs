#!/usr/bin/env node
/**
 * Convert jerry.fbx + locomotion animation FBX files into a single GLB with
 * all animations embedded and the embedded texture preserved.
 *
 * Mirrors convert-brain-fbx.mjs — both share the same Mixamo skeleton and
 * locomotion clip set. Kept as a separate script (not a parameterised one)
 * because the Three.js + sharp + gltf-transform globals stubbed at module
 * scope are awkward to reuse cleanly across hero pipelines.
 *
 * Reads:
 *   assets/source/jerry.fbx — base mesh + skeleton + embedded texture
 *   assets/source/<anim>.fbx — animation-only FBX files (explicit allowlist)
 *
 * Writes:
 *   assets/source/custom/jerry.glb  (picked up by optimize-custom-glbs.mjs)
 */
...
const BASE_FBX = path.join(SOURCE_DIR, 'jerry.fbx');
const OUTPUT = path.resolve('assets/source/custom/jerry.glb');
...
async function main() {
  if (!fs.existsSync(BASE_FBX)) {
    console.log(`No base mesh at ${BASE_FBX}, skipping jerry conversion.`);
    return;
  }
  ...
  if (textureBuffer) {
    ...
    const texture = doc.createTexture('jerry-diffuse')
