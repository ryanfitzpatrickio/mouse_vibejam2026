#!/usr/bin/env node
/**
 * Convert speedy.fbx + Mixamo locomotion FBX files into a single GLB. Mirrors
 * convert-jerry-fbx.mjs — Speedy is assumed to share the Mixamo skeleton used by
 * the other collection-unlock hero models.
 *
 * Reads:
 *   public/models/speedy.fbx        — base mesh + skeleton + embedded texture
 *   assets/source/<anim>.fbx     — animation-only FBX files (explicit allowlist)
 *
 * Writes:
 *   assets/source/custom/speedy.glb (picked up by optimize-custom-glbs.mjs)
 */
...
const BASE_FBX = path.resolve('public/models/speedy.fbx');
const OUTPUT = path.resolve('assets/source/custom/speedy.glb');
...
async function main() {
  if (!fs.existsSync(BASE_FBX)) {
    console.log(`No base mesh at ${BASE_FBX}, skipping speedy conversion.`);
    return;
  }
  ...
  if (textureBuffer) {
    ...
    const texture = doc.createTexture('speedy-diffuse')
